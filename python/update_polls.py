...
output_filename = "2014.Senate.polls.median.txt"
midtype = "median"
num_recent_polls_to_use = 3
archive_dir = "archive/"
state_numbers_filename = "state_numbers.txt"
...
bias_correction = 3
corrected_filename_parts = ("2014.Senate.polls.median.", "corrected.txt")
...
# expected format: state, dem, rep, assumption, note (dropped in read method), url
races_info_file = 'races.csv'
header_row = True
DEM_NAME, REP_NAME, ASSUMPTION, URL = tuple(range(4)) #Probably not necessary
...
def main():
    ...
    try:
        if sys.argv[1] == "--mean":
            midtype = "mean"
            output_filename = "2014.Senate.polls.mean.txt"
    except:
        pass

    races_info = read_races_info(races_info_file, header_row)

    # write the state indices to a file (in case we have to skip over a
    # state given in races.csv)
    with open(state_numbers_filename, 'w') as f:
        for num, state in one_indexed_enumerate(sorted(races_info.keys())):
            f.write("%2d %s\n" % (num, state))
...
def clean_polls(polls, day):
    ...
    if day < datetime.date(2014, 8, 1): # Before August 1
        n = datetime.timedelta(7 * 6, 0, 0) # 6 weeks
    elif day < datetime.date(2014, 9, 1):   # Month of August
        n = datetime.timedelta(7 * 4, 0, 0) # 4 weeks
    elif day < datetime.date(2014, 10, 1):  # September
    ...
    while True:
        try:
            ...
            if tries < 3:
                ...
                print "FAIL on page %d (tried three times)" % url
                sys.stdout.flush()
                raise
...
def fetch_latest_polls():
    ...
    for state, info in races_info.iteritems():
        ...
        reader = csv.reader(page, delimiter=',') # what is quotechar?

        # Get the first row, containing the names
        header = reader.next()
        header[DIFF] = 'Difference' # This was 'Undecided' in the csv from HuffPo

        # Determine whether the candidates are in the correct order (Dem, then Rep)
        dem_name = info[DEM_NAME]
        ...
        for row in reader:
            ...
            start_date = strpdate(row[START_DATE])
            end_date = strpdate(row[END_DATE])
            ...
            row[DIFF] = int(row[DEM]) - int(row[REP])

            # Store the info in the form:
            # (margin, start_date, end_date, mid_date, pop, poll_org, affil)
            races[state].append((row[DIFF], start_date, end_date, mid_date, row[NUM_OBS], pster, affil))
            rows.append(row)
    ...
    with open(csvfile, 'rb') as f:
        ...
        for row in reader:
            if not row[1] or not row[2]: # We only want races with two candidates given
                ...
                continue
            if row[5]:
                row[5] += '.csv' # generates the proper url of the csv file
            races_info[row[0]] = row[1:4] + row[5:] # we don't need the note
