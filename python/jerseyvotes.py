...
from update_polls import read_races_info, DEM_NAME, REP_NAME, URL
import csv
...
def display_state(jvdisplay, stateinfo):
    ...
    fmtstring = '<td>%s<a href="%s" style="color: %s; font-size: 9pt">%s +%s%%</a>%s</td>'
    if state == 'MT':
    ...
    if margin > 0:
        ...
        jvdisplay.write('<td>%s<a href="%s" style="color: black; font-size: 9pt">Tied</a>%s</td>' % (bold, data_url, endbold))
...
with open("../matlab/jerseyvotes.csv") as f:
    reader = csv.reader(f)
    for line in reader:
        line[2] = float(line[2])
        line[3] = float(line[3])
...
jvdisplay.write('<table id="jerseyvotes" width="100%" style="text-align: center; font-family: sans-serif">\n')
jvdisplay.write("<tr>\n")
